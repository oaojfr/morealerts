...
/**
 * Class PluginMorealertsTicketWaitingUser
 */
class PluginMorealertsTicketWaitingUser extends CommonDBTM {

   static $rightname = "plugin_morealerts";

   static function getTypeName($nb = 0) {
      return _n('Ticket waiting for user response', 'Tickets waiting for user response', $nb, 'morealerts');
   }

   static function cronInfo($name) {
      switch ($name) {
         case 'AdditionalalertsTicketWaitingUser':
            return [
               'description' => PluginMorealertsTicketWaitingUser::getTypeName(2)
            ];
            break;
      }
      return [];
   ...
   static function query($delay_waiting_user, $entity) {
      // Statut 4 = En attente de réponse utilisateur dans GLPI par défaut
      $delay_stamp = mktime(0, 0, 0, date("m"), date("d") - $delay_waiting_user, date("Y"));
   ...
   static function cronAdditionalalertsTicketWaitingUser($task = null) {
      global $DB, $CFG_GLPI;
      if (!$CFG_GLPI["notifications_mailing"]) {
         return 0;
      }
      $config = PluginMorealertsConfig::getConfig();
      $delay = $config->getDelayTicketWaitingUser();
      if ($delay <= 0) {
         return 0;
      }
      $entities = [$_SESSION["glpiactive_entity"] => $delay];
      $cron_status = 0;
      foreach ($entities as $entity => $delay_waiting_user) {
         $query = self::query($delay_waiting_user, $entity);
